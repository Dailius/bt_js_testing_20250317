...
app.post("/v1/users", async (req, res) => {
  ...
  try {
    if (!username || !email || !password) {
      return res
        .status(400)
        .json({ message: "username, email, password fields are mandatory." });
    }
    ...
    if (validateUsernameLength(user.username)) {
      return res
        .status(400)
        .json({ message: "Username length min 3 and max 35" });
    }
...
app.put("/v1/users/details", authenticate, async (req, res) => {
  const { username, email, password } = req.body;
  // const { id } = req.params;
  const identifiedUser = req.user;
  ...
  try {
    ...
    if (!validatePassword(user.password)) {
      return res.status(400).json({
        message:
          "Invalid password. It must be at least 8 characters long and include uppercase, lowercase, number, and special character.",
      });
    }
    ...
    const result = await pool.query(
      `
            ...
            RETURNING *;
            `,
      [user.username, user.email, hashedPassword, user.id]
    );
